...
def main():
    ...
    -----------
    annotate
        --correls : path to the input correlation table file
        --tree    : path to the input phylogenetic tree file
        --genome  : path to the genome metadata file
        --modules : path to the module definition directory or file
        --output  : path to the output annotation file
        --skip_kos: if present, skip the PD KO residual calculation
        --to_keep : optional file listing module thresholds to keep

    perms
        --correls : path to the input correlation table file
        --perms   : number of permutation replicates to run
        --procs   : number of worker processes to use
        --modules : path to the module definition directory
        --output  : path to the output directory for permutation files
        --skip_kos: if present, skip KO-specific permutation output
        --to_keep : optional file listing module thresholds to keep

    stats
        --correls : path to the input correlation table file
        --modules : path to the module definition directory
        --perms   : path to the permutation output directory
        --output  : path to the output directory for stats files and plots
        --skip_kos: if present, skip KO statistics
        --to_keep : optional file listing module thresholds to keep
    """
