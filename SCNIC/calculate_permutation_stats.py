"""
Compute permutation statistics and generate summary plots.

This module supports permutation-based p-value statistics and writes
text and plot outputs for SCNIC module analyses.
"""
...
def p_adjust(pvalues, method='fdr_bh'):
    ...
    method : str, default 'fdr_bh'
        Correction method for statsmodels.multipletests.
...
def get_perms(perms_loc):
    """
    Load permutation result matrices from disk.

    Parameters
    ----------
    perms_loc : str
        Glob pattern matching permutation result files.

    Returns
    -------
    pandas.DataFrame
        Combined permutation results across files.
    """
    frame_list = list()
...
def perm_mannwhitneyu(x, y, dist, alternative):
    """
    Compute a Mann-Whitney U permutation statistic and empirical p-value.
    ...
    -------
    tuple
        Test statistic and empirical p-value.
    """
...
def perm_ttest_ind(x, y, dist, alternative='two_sided'):
    """
    Compute an independent t-test statistic and empirical permutation p-value.
    ...
    -------
    tuple
        t-statistic and empirical p-value.
    """
...
def get_stats(correls, modules_across_rs, pd_perms, pd_ko_perms=None):
    """
    Compute statistics for each module across correlation thresholds.

    Parameters
    ----------
    correls : pandas.DataFrame
        Correlation results including module membership.
    modules_across_rs : dict
        Mapping of min_r keys to module dictionaries.
    pd_perms : pandas.DataFrame
        Permutation results for PD statistics.
    pd_ko_perms : pandas.DataFrame or None, optional
        Permutation results for PD KO statistics.

    Returns
    -------
    pandas.DataFrame
        Summary statistics dataframe with p-values and adjusted p-values.
    """
    stats_dfs = list()
...
def tabulate_stats(stats, modules_across_rs, alphas=(.01, .05, .1, .15, .2)):
    """
    Summarize module p-value statistics at specified significance thresholds.

    Parameters
    ----------
    stats : pandas.DataFrame
        Statistics generated by get_stats.
    modules_across_rs : dict
        Mapping of correlation thresholds to modules.
    alphas : tuple of float, default (.01, .05, .1, .15, .2)
        Significance levels to tabulate.

    Returns
    -------
    pandas.DataFrame
        Tabulated summary dataframe.
    """
    module_count = list()
    r_values = list()
...
def make_plots(stats, tab_stats, output_loc, alphas=(.01, .05, .1, .15, .2)):
    """
    Generate scatter and boxplot visualizations for permutation statistics.

    Parameters
    ----------
    stats : pandas.DataFrame
        Detailed statistics for modules.
    tab_stats : pandas.DataFrame
        Tabulated summary statistics.
    output_loc : str
        Directory where plots will be saved.
    alphas : tuple of float, default (.01, .05, .1, .15, .2)
        Significance thresholds to plot.
    """
    for alpha in alphas:
        # pd_plot
        ...
        ## madi update: taking out seaborn entirely since its incompatible with newer versions of matplotlib
        ## and has not been updated 
        fig, ax = plt.subplots(figsize=[13, 3])
        ax.scatter(tab_stats['minr'], tab_stats[f'pd_percent_sig_{alpha}'],
                   s=tab_stats.module_count)
        ax.set_xlabel('minr')
        ax.set_ylabel(f'pd_percent_sig_{alpha}')
        plt.savefig(join(output_loc, f'pd_sig_plot_{alpha}.png'))
        plt.close(fig)

        fig, ax = plt.subplots(figsize=[13, 3])
        ax.scatter(tab_stats['minr'], tab_stats[f'pd_ko_percent_sig_{alpha}'],
                   s=tab_stats.module_count)
        ax.set_xlabel('minr')
        ax.set_ylabel(f'pd_ko_percent_sig_{alpha}')
        plt.savefig(join(output_loc, f'pd_ko_sig_plot_{alpha}.png'))
        plt.close(fig)
   ...
   ## madi update: same as above!
    groups = sorted(stats.r_level.unique())
    values = [stats.loc[stats.r_level == level, 'pd_ko_adj_pvalue'].dropna().tolist()
              for level in groups]
    fig, ax = plt.subplots(figsize=[13, 3])
    ax.boxplot(values, tick_labels=groups, orientation='vertical')
    ax.set_xlabel('r_level')
    ax.set_ylabel('pd_ko_adj_pvalue')
    plt.savefig(join(output_loc, 'pd_ko_pvalue_boxplots.png'))
    plt.close(fig)

    values = [stats.loc[stats.r_level == level, 'pd_adj_pvalue'].dropna().tolist()
              for level in groups]
    fig, ax = plt.subplots(figsize=[13, 3])
    ax.boxplot(values, tick_labels=groups, orientation='vertical')
    ax.set_xlabel('r_level')
    ax.set_ylabel('pd_adj_pvalue')
    plt.savefig(join(output_loc, 'pd_pvalue_boxplots.png'))
    plt.close(fig)
...
def do_stats(correls_loc, modules_directory_loc, perms_loc, output_loc, skip_kos=False, to_keep_loc=None,
             alphas=(.01, .05, .1, .15, .2)):
    """
    Run the full permutation statistics workflow and save outputs.

    Parameters
    ----------
    correls_loc : str
        Path to correlation table file.
    modules_directory_loc : str
        Path or glob pattern for module files.
    perms_loc : str
        Directory containing permutation outputs.
    output_loc : str
        Directory for stats files and plots.
    skip_kos : bool, default False
        Whether to skip PD KO statistics.
    to_keep_loc : str or None, optional
        Path to a file listing modules to keep.
    alphas : tuple of float, default (.01, .05, .1, .15, .2)
        Significance thresholds for summary tables.
    """
